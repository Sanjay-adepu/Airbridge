...
const UploadInterface = () => {
  ...
  const handleSubmit = async () => {
    ...
    if (selectedType === 'files') {
      ...
      const zip = new JSZip();
      files.forEach((file) => {
        const folder = file.webkitRelativePath?.split('/').slice(1, -1).join('/');
        folder ? zip.folder(folder)?.file(file.name, file) : zip.file(file.name, file);
      });

      const zippedBlob = await zip.generateAsync({ type: 'blob' });
      formData.append('zip', zippedBlob, `${sessionId}.zip`);
